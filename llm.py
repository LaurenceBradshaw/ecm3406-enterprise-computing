...
def llm() -> tuple[dict[str, str], int]:
    ...
    mistral_rsp, success = make_request(MISTRAL_API_URL, "POST", res, payload, headers)
    if not success:
        return res.to_tuple()
    
    # Parse response
    if (mistral_rsp is None \
        or "choices" not in mistral_rsp \
        or len(mistral_rsp["choices"]) == 0 \
        or "message" not in mistral_rsp["choices"][0] \
        or "content" not in mistral_rsp["choices"][0]["message"]
        ):
        res.status_code = codes.internal_server_error # or bad_gateway?
        return res.to_tuple()
    
    output_text = mistral_rsp["choices"][0]["message"]["content"]
    # Return output
